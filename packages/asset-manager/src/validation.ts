 ...
 * @returns true if the parameters are from a mod user
 */
export type FromModUserTestFunc = (param: AppearanceUpdateParameters) => boolean;

let isFromModUser: FromModUserTestFunc | undefined = undefined;
...
function runHook() {
    ...
    HookManager.hookFunction('ValidationResolveAppearanceDiff', 1, (args, next) => {
        if (isFromModUser) (args[3] as AUParametersExt).fromModUser = isFromModUser(args[3]);
        return next(args);
    });
}
